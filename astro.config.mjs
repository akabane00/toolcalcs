  ...
  integrations: [sitemap({
    ...
    serialize(item) {
      ...
      if (url === 'https://toolcalcs.com/' ||
          url.match(/toolcalcs\.com\/(financial|health|math|date-time|conversion|everyday|construction|astrology|insurance|tax|real-estate|business|education|automotive)-calculators\/$/) ||
          url.match(/toolcalcs\.com\/(test|tools|blog|worksheets)\/$/)) {
        item.priority = 1.0;
        item.changefreq = 'weekly';
      }
      // High: individual calculators, games, tools, worksheets, blog articles
      else if (url.match(/toolcalcs\.com\/(financial|health|math|date-time|conversion|everyday|construction|astrology|insurance|tax|real-estate|business|education|automotive)-calculators\/[^/]+\/$/) ||
